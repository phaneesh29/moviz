...
export default function TvPageClient({ id, initialSeason, initialEpisode }: TvPageClientProps) {
  ...
  }, [activeProvider, id, selectedSeason, selectedEpisode, router]);
  ...
  useEffect(() => {
    ...
    const fetchStaticData = async () => {
      ...
      try {
        const [seriesRes, recRes, videosRes] = await Promise.all([
  ...
  useEffect(() => {
    ...
    const fetchEpisodeData = async () => {
      try {
        const [seasonRes, episodeRes, creditsRes] = await Promise.all([
          ...
          fetch(`/api/tv/${id}/season/${selectedSeason}/episode/${selectedEpisode}/credits`, { signal: controller.signal }),
        ]);
  ...
  const activeMediaTitle = episode?.name
    ? `${series.name} S${selectedSeason}E${selectedEpisode} ${episode.name}`
    : series.name;
